...
5. 点击顶栏的“Choose Folder & Export”，选择一个普通父目录。Framecut 会自动在其中建立时间戳子文件夹并写入全部成品。不要选择“下载”、用户主目录或“应用程序”等受保护的根目录；Chrome 会在页面拿到目录权限前拦截它们。
...
- 画框工具：拖动建立新裁切框，快捷键 `D`
- 画完后自动进入选择状态，新框会保持选中并显示四角控制点
- 点击已有裁切框会直接选中并进入选择工具；画框工具下仍可从框内空白处开始绘制重叠框
- 选择工具：拖动裁切框移动，拖动空白处平移底图，点击空白处取消选择，四角控制点用于缩放，快捷键 `V`
- 手形工具：平移画布，快捷键 `H`
- 复制框按钮：在选中框附近建立一个不重叠的同尺寸新框，并立即选中新框
- `Command/Ctrl + C`、`Command/Ctrl + V`：复制并粘贴当前裁切框
- 按住空格拖动：从任意工具临时平移底图
- 放大、缩小和还原位于单独的缩放面板；鼠标滚轮也可缩放画布
- `Delete` / `Backspace`：删除当前裁切框
- 点击右侧清单中的编号或名称：选择对应裁切框
- 裁切框较多时，右侧清单可独立滚动；输出数量、预计大小、按钮和结果提示位于顶栏

每次输出会建立类似 `framecut-20260727-090503` 的新文件夹；其中的文件依次命名为 `framecut-20260727-090503-01.tif`、`framecut-20260727-090503-02.tif`。如果同一秒内已有同名文件夹，Framecut 会自动追加序号，不会覆盖原有输出。

## 输出方式

桌面版 Chrome 会通过 File System Access API 获得用户选择的父目录，在其中自动建立本次输出的子文件夹。各裁切框按顺序编码、写盘，不会等全部成品都完成后再落盘；这是大量照片时的推荐方式。

自动建立子文件夹不能绕过 Chrome 对敏感根目录的保护。浏览器会先判断用户选择的父目录能否授权，只有授权成功后页面才能调用创建子目录的 API；因此“下载”等根目录仍不能直接选择。可以在这些位置预先建立一个普通目录（例如“下载/Framecut 输出”），以后选择这个目录，Framecut 再在其中自动按批次建子目录。
